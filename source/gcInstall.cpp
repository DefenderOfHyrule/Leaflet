...
namespace inst::gc {
    ...
    static u64 GetReclaimableBytes(const std::vector<inst::gc::direct::GameCardContentEntry>& entries,
                                    const std::vector<size_t>& selection, NcmStorageId destStorageId) {
        u64 reclaimableBytes = 0;
        for (size_t idx : selection) {
            if (idx >= entries.size()) continue;
            const NcmContentMetaKey& wantedKey = entries[idx].key;
            ...
            for (NcmStorageId sid : { NcmStorageId_SdCard, NcmStorageId_BuiltInUser }) {
                ...
                if (R_SUCCEEDED(ncmContentMetaDatabaseGetLatestContentMetaKey(&db, &key, wantedKey.id))
                    && key.type == wantedKey.type) {
                    existingKey = key;
                    existingSid = sid;
                    foundExisting = true;
                }
            ...
            if (!foundExisting || existingSid != destStorageId) continue;

            NcmContentMetaDatabase db = {};
            if (R_FAILED(ncmOpenContentMetaDatabase(&db, existingSid))) continue;
            NcmContentStorage cs = {};
            const bool hasStorage = R_SUCCEEDED(ncmOpenContentStorage(&cs, existingSid));
            ...
            while (true) {
                s32 got = 0;
                if (R_FAILED(ncmContentMetaDatabaseListContentInfo(&db, &got, infos, kBatch, &existingKey, offset)))
                    break;
                if (got == 0) break;
                for (s32 j = 0; j < got; j++) {
                    bool has = !hasStorage;
    ...
    static void RunInstall(int storageChoice, const std::vector<size_t>& selection,
        ...
        try {
            ...
            if (!success) {
                const std::string reason = inst::gc::direct::GetLastError();
                if (!reason.empty())
                    THROW_FORMAT("Gamecard install failed: %s", reason.c_str());
                else
    ...
    void InstallFromGamecard(int storageChoice) {
        ...
        const NcmStorageId destStorageId = (storageChoice == 0) ? NcmStorageId_SdCard : NcmStorageId_BuiltInUser;
        const u64 reclaimableBytes = GetReclaimableBytes(entries, all, destStorageId);
        requiredBytes = (requiredBytes > reclaimableBytes) ? (requiredBytes - reclaimableBytes) : 0;
    ...
    void InstallSelectedFromGamecard(int storageChoice, uint32_t contentMask) {
        ...
        const NcmStorageId destStorageId = (storageChoice == 0) ? NcmStorageId_SdCard : NcmStorageId_BuiltInUser;
        const u64 reclaimableBytes = GetReclaimableBytes(entries, selection, destStorageId);
        requiredBytes = (requiredBytes > reclaimableBytes) ? (requiredBytes - reclaimableBytes) : 0;
