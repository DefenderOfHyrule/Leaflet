...
namespace inst::ui {
    ...
    MainPage::MainPage() : Layout::Layout() {
        ...
        if (inst::config::oledMode) {
            cardBase = COLOR("#1A1A1AFF");
            ringColor = COLOR("#FF4D4DFF");
            paneColor = COLOR("#101010FF");
        } else {
    ...
    static bool warnIfPatchesMissing() {
        ...
        if (!inst::util::checkSigPatches()) {
            mainApp->CreateShowDialog(
                "Signature patches not detected",
                "fs-patch does not appear to be active or its patches were not applied.\n\nfs and ldr must be patched before installing titles.\n\nEnsure fs-patch is installed and has run at least once, then restart and try again.",
                {"common.ok"_lang},
                true
            );
            return false;
