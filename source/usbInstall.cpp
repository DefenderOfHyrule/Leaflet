...
namespace usbInstStuff {
    ...
    void installTitleUsb(std::vector<std::string> ourTitleList, int ourStorage)
    {
        ...
        inst::diag::StartSession("quark_usb", ourTitleList.size());
        ...
        try {
            for (fileItr = 0; fileItr < ourTitleList.size(); fileItr++) {
                ...
                inst::ui::instPage::setTopInstInfoText(
                    "inst.info_page.top_info0"_lang + currentName + "inst.usb.source_string"_lang);
                ...
                if (remotePath.size() >= 3 && remotePath.compare(remotePath.size() - 3, 2, "xc") == 0) {
                    auto xci = std::make_shared<quark::install::QuarkXCI>(remotePath);
                    installTask = std::make_unique<leaf::install::xci::XCIInstallTask>(
                        destStorage, inst::config::ignoreReqVers, xci);
                } else {
                    auto nsp = std::make_shared<quark::install::QuarkNSP>(remotePath);
                ...
                installTask->m_suppressReinstallPrompt = true;
                installTask->Prepare();

                if (installTask->reinstallDetected) {
                    ...
                    installTask->reinstallDetected   = false;
                    installTask->m_skipReinstallCheck = true;
                    installTask->Prepare();
                }
