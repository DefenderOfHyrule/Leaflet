...
static void ncaReader(NcaCtx* c) {
    ...
    for (uint64_t done = c->written; done < c->total; ) {
        ...
        if (got < chunk) memset(rb.get()+got, 0, chunk-got);
        { std::unique_lock<std::mutex> lk(c->mtx);
          c->canR.wait(lk, [c]{ return c->bufSz == 0 || c->cancelled; });
          if (c->cancelled) return;
...
static void ncaWriter(NcaCtx* c) {
    while (c->written < c->total) {
        ...
        size_t chunk;
        { std::unique_lock<std::mutex> lk(c->mtx);
          c->canW.wait(lk, [c]{ return c->bufSz > 0 || c->cancelled; });
          if (c->cancelled) return;
          chunk = c->bufSz; }
        Result rc = ncmContentStorageWritePlaceHolder(&c->cs, &c->pid, c->written, c->buf, chunk);
        if (R_FAILED(rc)) { gcLog("write fail 0x%x\n", rc); c->err = true; return; }
        c->written += chunk;
...
static bool InstallNca(const std::string& path, const NcmContentId& cid,
    ...
    if (h.size > GC_NCA_HEADER_SIZE) {
        ...
        while (ctx.written < ctx.total && !ctx.err) {
            ...
            char pb[160];
            snprintf(pb, sizeof(pb), "%d%% • %.1f MB/s • %s",
                     (int)(pct + 0.5f), lastMbps, name.c_str());
...
bool InstallSelectedFromGamecard(const std::string& mountPath, NcmStorageId storageId,
    ...
    for (auto& parsed : selectedParsed) {
        ...
        if (foundExisting) {
            ...
            const char* locStr = (existingSid == NcmStorageId_SdCard) ? "SD card" : "system memory";

            if (existingKey.version == parsed.key.version && !inst::config::autoSkipReinstall) {
                char msg[256];
            ...
            // if replacing a Patch, reset the AVM required version floor
            // before removing the old NCAs
            if (parsed.key.type == NcmContentMetaType_Patch) {
                const u64 baseId = AppIdFromTitle(parsed.key.id, parsed.key.type);
                if (R_SUCCEEDED(avmInitialize())) {
                    ...
                    if (R_SUCCEEDED(avmGetVersionListImporter(&importer))) {
                        AvmVersionListEntry entry = {
                            .application_id = baseId,
                            .version        = parsed.key.version,
                            .required       = parsed.key.version,
                        };
                        avmVersionListImporterSetTimestamp(&importer, AvmCurrentTimestamp());
                        avmVersionListImporterSetData(&importer, &entry, 1);
                        avmVersionListImporterFlush(&importer);
                        avmVersionListImporterClose(&importer);
                    }
                    avmExit();
                }
                nsextPushLaunchVersion(baseId, parsed.key.version);
                gcLog("reset AVM floor for %016lX to v%u\n", baseId, parsed.key.version);
            }

            // delete existing content
            gcLog("deleting existing %s from storage %u\n", typeStr, existingSid);
            NcmContentMetaDatabase db = {};
            if (R_SUCCEEDED(ncmOpenContentMetaDatabase(&db, existingSid))) {
                NcmContentStorage cs = {};
    ...
    // set content meta database for selected entries
    for (auto& parsed : selectedParsed) {
...

    {
        ...
        for (auto& parsed : allParsed) {
            ...
            for (auto& sel : selectedParsed) {
                if (sel.key.id == parsed.key.id && sel.key.type == parsed.key.type)
            ...
            if (isSelected) {
                ...
                // include only if already installed digitally
                for (NcmStorageId sid : { NcmStorageId_SdCard, NcmStorageId_BuiltInUser }) {
                    NcmContentMetaDatabase db = {};
                    if (R_FAILED(ncmOpenContentMetaDatabase(&db, sid))) continue;
                    NcmContentMetaKey key = {};
                ...
                // if not installed anywhere, don't include it in the record
            }
        }

        s32 existingCount = 0;
        nsCountApplicationContentMeta(appId, &existingCount);
        Result delRc = nsDeleteApplicationRecord(appId);
        gcLog("delete record: 0x%x\n", delRc);
        ...
        // set AVM launch version and required floor.
        // installing only the base game explicitly clears any previously set
        // higher requirement (which would otherwise force "update required").
        // the floor is set to the highest version among the selected entries:
        // base-only > 0, update included > patch version.
        {
            const u64 appId2  = AppIdFromTitle(allParsed[0].key.id, allParsed[0].key.type);
            const u64 patchId = appId2 | 0x800ULL;
            nsextPushLaunchVersion(appId2, maxVersion);
            gcLog("AVM: setting version=%u required=%u for appId=%016lx patchId=%016lx\n",
                  maxVersion, maxVersion, appId2, patchId);
            Result avmRc = avmInitialize();
            if (R_SUCCEEDED(avmRc)) {
                avmPushLaunchVersion(appId2, maxVersion);
                AvmVersionListImporter importer;
                Result impRc = avmGetVersionListImporter(&importer);
                gcLog("AVM: avmGetVersionListImporter rc=0x%x\n", impRc);
                if (R_SUCCEEDED(impRc)) {
                    u64 ts = AvmCurrentTimestamp();
                    gcLog("AVM: timestamp=%lu\n", ts);
                    AvmVersionListEntry entries[2] = {
                        { .application_id = appId2,  .version = maxVersion, .required = maxVersion },
                        { .application_id = patchId, .version = maxVersion, .required = maxVersion },
                    };
                    Result tsRc = avmVersionListImporterSetTimestamp(&importer, ts);
                    Result dtRc = avmVersionListImporterSetData(&importer, entries, 2);
                    Result flRc = avmVersionListImporterFlush(&importer);
                    gcLog("AVM: SetTimestamp=0x%x SetData=0x%x Flush=0x%x\n", tsRc, dtRc, flRc);
                    avmVersionListImporterClose(&importer);
                }
                avmExit();
            } else {
                gcLog("AVM: avmInitialize failed rc=0x%x\n", avmRc);
            }
        }
    }
    ...
    // install NCAs for selected entries
        ...
        for (size_t ni = 0; ni < parsed.contentInfos.size(); ni++) {
            ...
            { NcmContentStorage cs = {};
              if (R_SUCCEEDED(ncmOpenContentStorage(&cs, storageId))) {
                  ...
                  ncmContentStorageClose(&cs);
                  if (has) { gcLog("NCA already present, skipping\n"); continue; }
              }
            }
