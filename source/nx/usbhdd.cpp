...
namespace nx::hdd
{
    ...
    u8 getFsType(const std::string& mountName)
    {
        std::scoped_lock lock(g_mutex);
        const u32 cnt = usbHsFsGetMountedDeviceCount();
        for (u32 i = 0; i < cnt && i < kMaxDevices; i++) {
            const std::string devName = g_usbDevices[i].name;
            const auto colon = devName.find(':');
            const std::string devMount = (colon != std::string::npos) ? devName.substr(0, colon) : devName;
            const auto qcolon = mountName.find(':');
            const std::string qMount = (qcolon != std::string::npos) ? mountName.substr(0, qcolon) : mountName;
            if (devMount == qMount) return g_usbDevices[i].fs_type;
        }
        return 0;
    }
