...
namespace quark {
    ...
    namespace {
        ...
        std::string GenerateRandomId() {
            ...
            return std::string(kAdjectives[adjIdx]) + "-" + std::string(kNouns[nounIdx]);
        }
    ...
    void InitConsoleId() {
        if (inst::config::isValidConsoleId(inst::config::consoleId)) {
            g_consoleId = inst::config::consoleId;
            return;
        }

        g_consoleId = GenerateRandomId();
        inst::config::consoleId = g_consoleId;
        inst::config::consoleIdIsCustom = false;
    ...
    bool SetCustomConsoleId(const std::string& id) {
        if (!inst::config::isValidConsoleId(id)) return false;
        g_consoleId = id;
        inst::config::consoleId = id;
        inst::config::consoleIdIsCustom = true;
        inst::config::setConfig();
        return true;
    }
    ...
    void InitNetConsoleId() {
        if (g_consoleId.empty()) InitConsoleId();
        g_netConsoleId = g_consoleId;
    }
