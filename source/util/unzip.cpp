...
namespace inst::zip {
    ...
    bool extractNroToPath(const std::string zipFile, const std::string outPath) {
        unzFile unz = unzOpen(zipFile.c_str());
        if (!unz) return false;

        int code = unzGoToFirstFile(unz);
        while (code == UNZ_OK) {
            unz_file_info_s* fileInfo = _getFileInfo(unz);
            std::string name = _getFullFileName(unz, fileInfo);
            free(fileInfo);

            if (name.size() >= 4 && name.substr(name.size() - 4) == ".nro") {
                unz_file_info_s* fi = _getFileInfo(unz);
                int result = _extractFile(outPath.c_str(), unz, fi);
                free(fi);
                unzClose(unz);
                return result == 0;
            }
