...
namespace usb {
    ...
    namespace {
        ...
        Result TransferImpl(void *buf, const size_t size, UsbDsEndpoint *ep) {
            ...
            while(true) {
                s32 idx = 0;
                rc = waitMulti(&idx, UINT64_MAX,
                    waiterForEvent(&ep->CompletionEvent),
            ...
            if(R_SUCCEEDED(rc) && report_size != size) {
                return MAKERESULT(Module_Libnx, LibnxError_BadUsbCommsRead);
            }
            return rc;
        }
            ...
            if(!g_Initialized) {
                ...
                if(R_SUCCEEDED(rc)) {
                    if(hosversionAtLeast(5,0,0)) {
                        ...
                        // Send manufacturer
                        if(R_SUCCEEDED(rc)) {
                            rc = usbDsAddUsbStringDescriptor(&iManufacturer, "DefenderOfHyrule");
                        }
                        // Send product
                        if(R_SUCCEEDED(rc)) {
                            rc = usbDsAddUsbStringDescriptor(&iProduct, "Leaflet");
                        }
                        // Send serial number — format: "VERSION/ConsoleId"
                        if(R_SUCCEEDED(rc)) {
                            quark::InitConsoleId();
                            const std::string serialStr = std::string(GLEAF_VERSION) + "/" + quark::GetConsoleId();
                            rc = usbDsAddUsbStringDescriptor(&iSerialNumber, serialStr.c_str());
                        }
