...
namespace pu::ui
{
    namespace
    {
        ...
        Color TextColorFor(const Color& surface)
        {
            return IsLightColor(surface) ? Color(26, 26, 26, 255) : Color(235, 235, 235, 255);
        }

        Color WithAlpha(const Color& c, u8 a)
        {
            return Color(c.R, c.G, c.B, a);
        }
    }

    Color Dialog::s_defaultBackground = { 225, 225, 225, 255 };
    Color Dialog::s_defaultBorder     = { 0, 0, 0, 0 };
    Color Dialog::s_defaultAccent     = { 180, 180, 200, 255 };

    void Dialog::SetDefaultColors(const Color& Background, const Color& Border, const Color& Accent)
    {
        s_defaultBackground = Background;
        s_defaultBorder = Border;
        s_defaultAccent = Accent;
    }
        ...
        : stitle(Title), scnt(Content)
    {
        ...
        this->title = render::RenderText(this->titleFont, this->titleFontEx, Title, TextColorFor(s_defaultBackground));
        this->cnt = render::RenderText(this->contentFont, this->contentFontEx, Content, WithAlpha(TextColorFor(s_defaultBackground), 230));
        this->osel = 0;
        this->prevosel = 0;
        this->selfact = 255;
        this->pselfact = 0;
    ...
    Dialog::~Dialog()
{
    ...
    {
        this->opts.push_back(render::RenderText(this->optionsFont, this->optionsFontEx, Name, TextColorFor(s_defaultBackground)));
    ...
    s32 Dialog::Show(render::Renderer::Ref &Drawer, void *App)
    {
        ...
        s32 r = 35;
        s32 nr = s_defaultAccent.R;
        s32 ng = s_defaultAccent.G;
        s32 nb = s_defaultAccent.B;
        bool end = false;
        s32 initfact = 0;
        ...
        while(true)
        {
            bool ok = app_ref->CallForRenderWithRenderOver([&](render::Renderer::Ref &Drawer) -> bool
            {
                ...
                // s32 fw = bw - (r * 2);
                // s32 fh = bh - (r * 2);
                Color clr = { s_defaultBackground.R, s_defaultBackground.G, s_defaultBackground.B, (u8)((s_defaultBackground.A * initfact) / 255) };
                s32 aclr = initfact;
                if(aclr < 0) aclr = 0;
                ...
                if(s_defaultBorder.A > 0)
                {
                    Color borderClr = { s_defaultBorder.R, s_defaultBorder.G, s_defaultBorder.B, (u8)((s_defaultBorder.A * initfact) / 255) };
                    Drawer->RenderRoundedRectangle(borderClr, dx, dy, bw, bh, r);
                }
                ...
                for(s32 i = 0; i < this->opts.size(); i++)
                {
                    ...
                    s32 ry = ely;
                    s32 rr = (elemh / 2);
                    Color dclr = { nr, ng, nb, (u8)((s_defaultAccent.A * initfact) / 255) };
                    if(this->osel == i)
                    {
                        if(this->selfact < 255)
                        {
                            dclr = { nr, ng, nb, (u8)((s_defaultAccent.A * this->selfact) / 255) };
                            Drawer->RenderRoundedRectangleFill(dclr, rx, ry, elemw, elemh, rr);
                            this->selfact += 48;
                        }
                        else
                        {
                            dclr = { nr, ng, nb, (u8)((s_defaultAccent.A * initfact) / 255) };
                            Drawer->RenderRoundedRectangleFill(dclr, rx, ry, elemw, elemh, rr);
                        }
                    }
                    else if(this->prevosel == i)
                    {
                        if(this->pselfact > 0)
                        {
                            dclr = { nr, ng, nb, (u8)((s_defaultAccent.A * this->pselfact) / 255) };
                            Drawer->RenderRoundedRectangleFill(dclr, rx, ry, elemw, elemh, rr);
                            this->pselfact -= 48;
                        }
                    }
